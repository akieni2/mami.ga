...
class RideLiveTrackingNotifier extends StateNotifier<RideLiveTrackingState> {
  ...
  void _listenWebSocket() {
    ...
    void onLocation(String event, Map<String, dynamic> payload) {
      ...
      final targetLat = payload['target_latitude'] as num?;
      final targetLng = payload['target_longitude'] as num?;

      List<LatLng> route = state.route;
      if (targetLat != null && targetLng != null) {
        route = RouteUtils.straightLine(
          driverPos,
          LatLng(targetLat.toDouble(), targetLng.toDouble()),
        );
      }
      ...
      state = state.copyWith(
        ...
        distanceKm: (payload['distance_to_client_km'] as num?)?.toDouble() ??
            state.distanceKm,
        etaMinutes:
            (payload['eta_minutes'] as num?)?.toInt() ?? state.etaMinutes,
      );
    }

    reverb.subscribeRide(_rideId, onLocation);
  }
